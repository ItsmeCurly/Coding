    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource().equals(cb)) {
            ...
            switch (command) {
                case 0:
                    try {
                        if (argsText.trim().length() != "k".length()) throw new Gui.InvalidArgsException();
                        st = new StringTokenizer(argsText, " ");
                        key = Integer.parseInt(st.nextToken());
                        if (key < 2) throw new Gui.InvalidKValException();
                    } catch (NoSuchElementException | NumberFormatException | Gui.InvalidArgsException err) {
                        displayCaption("Command requires args in form c k");
                        break;
                    } catch (Gui.InvalidKValException err) {
                ...
                case 1:
                    try {
                        if (argsText.trim().length() != "k d".length()) throw new Gui.InvalidArgsException();
                        st = new StringTokenizer(argsText, " ");
                        key = Integer.parseInt(st.nextToken());
                        data = st.nextToken();
                        if (rm.getKst() == null)
                    ...
                    executed = true;
                    rm.store(new Gui.TreeNode(key, data));
                ...
                case 2:
                    try {
                        ...
                        if (rm.getKst() == null)
                            throw new NullPointerException();
                    } catch (NoSuchElementException | Gui.InvalidArgsException err) {
                ...
                case 3:
                    ...
                    executed = true;
                    Gui.TreeNode find = rm.searchNode(key);
                    if (find != null) {
                        data = find.getData();
                        printCommand();
